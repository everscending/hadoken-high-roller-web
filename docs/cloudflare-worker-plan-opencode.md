...
- [ ] Create `tsconfig.worker.json` with Cloudflare types
...
hadoken-high-roller/
├── docs/
│   └── cloudflare-worker-plan-opencode.md    # This document
├── src/
│   ├── lib/                                  # Shared code (both targets)
│   │   ├── database/
│   │   │   ├── adapter.ts                    # DB interface definition
...
│   │   └── database-adapter-electron.ts      # Electron DB adapter (extracted)
│   ├── preload/                              # Electron-only (preload script)
│   │   ├── index.ts                          # IPC bridge (minimal changes)
...
| `src/preload/index.ts` | Minimal changes; re-exports from shared types |
...
| `package.json` | New scripts (`build:worker`, `dev:worker`) and dev dependencies (`wrangler`, `@cloudflare/workers-types`) |
...
| Electron build config | `electron.vite.config.ts` - Electron build untouched |
| Desktop packaging | `electron-builder.yml` - unchanged |
